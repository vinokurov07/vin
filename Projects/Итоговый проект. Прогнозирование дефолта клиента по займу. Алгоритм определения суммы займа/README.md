...
### Результаты
[Весь проект выполнен в ноутбуке](</Projects/Project 7. Прогнозирование дефолта клиента по займу. Алгоритм определения суммы займа/Project 7.ipynb>) 
...
В задаче кредитного скоринга ключевой является способность модели к ранжированию заемщиков по уровню риска, так как на основе вероятности дефолта принимаются решения о размере кредитного лимита и условиях выдачи. Метрика ROC-AUC идеально отражает эту способность, показывая, насколько хорошо модель отделяет надежных заемщиков от ненадежных независимо от выбранного порога классификации. Поэтому при выборе модели в первую очередь будем ориентироваться на нее.  
...
В нашем случае лучшие метрики качества показывает модели градиентого бустинга и случайного леса показывают похожие результаты, но первая показывает меньшее переобучение, остановимся на ней.
Итак, что можно сказать по ней:
* **AUC-ROC и Gini** 
Метрики ROC-AUC = 0.76 и Gini: 0.53 говорят о том, модель хорошо ранжирует клиентов по риску, т.е. модель можно использовать для сортировки клиентов на группы риска и использовать дифференцированный подход при выдаче займов (если взять случайного покупателя и случайного непокупателя, то в 76 % случаев модель присвоит покупателю более высокую вероятность, чем непокупателю). 
* **F1-score**   
 F1-score = 0.45, что говорит о хорошем что балансе между Precision и Recall. 
* **PR-AUC**   
В условиях несбалансированного распределения эта метрика более актуальна, т.к. в отличие от F1-score учитвает все возможные пороги вероятности (интеграл под всей PR-кривой). Знчение = 0,42 говорит о том, что в целом модель имеет умеренное, но стабильное качество обнаружения дефолтов при любых порогах.
* **Recall**   
Подобрав веса классов удалось достичь хороших резудьтатов по выявлению дефолтных клиентов. Recall = 0.71. О чем это говорит? Из всех клиентов модель верно определяет 71%. Хороший результат. 
* **Precision**   
Сбалансировав веса классов мы повысили охват, но точность снизилась до 0.33. Это закономерный процесс, стало больше значений, из которых нужно выбирать True Positive, но по сути когда модель говорит "дефолт", она права только в 33% случаев, а 67% добросоветсных клиентов получают отказ. Попробуем исправить это изменением порога вероятности отнесения к классу 1.
